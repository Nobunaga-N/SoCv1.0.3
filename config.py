...
Обновленная версия с исправленными координатами сезонов и серверов.
"""
import os
...
COORDINATES = {
    ...
    # ИСПРАВЛЕННЫЕ координаты для скроллинга
    'season_scroll_start': (257, 550),    # Уменьшена Y с 573 до 550
    'season_scroll_end': (257, 200),      # Увеличена Y с 240 до 200
    'server_scroll_start': (640, 550),    # Центр области серверов
    'server_scroll_end': (640, 200),      # Увеличена Y с 100 до 200

    # ИСПРАВЛЕННЫЕ координаты сезонов на основе изображения
    'seasons': {
    ...
    'servers': {
        'left_column_x': 500,    # X-координата левого столбца серверов
        'right_column_x': 900,   # X-координата правого столбца серверов
...
OCR_REGIONS = {
    'seasons': (150, 150, 220, 400),    # Область для поиска текста сезонов
    'servers': (400, 150, 500, 450),    # Область для поиска текста серверов
    'skip_button': (1040, 12, 200, 60)  # Область для поиска кнопки ПРОПУСТИТЬ
}
...
SERVER_RECOGNITION_SETTINGS = {
    'max_scroll_attempts': 5,        # Максимальное количество попыток скроллинга
    'scroll_duration': 1000,         # Продолжительность скроллинга в мс
    'recognition_attempts': 3,       # Количество попыток распознавания OCR
    'servers_per_screen': 8,         # Примерное количество серверов на экране
}
...
def validate_config():
    ...
    if not IMAGES_DIR.exists():
        print(f"Предупреждение: Директория изображений не существует: {IMAGES_DIR}")
        return False
    ...
    for season_id, season_data in SEASONS.items():
        if season_data['min_server'] < season_data['max_server']:
            print(f"Ошибка: В сезоне {season_id} min_server меньше max_server")
            return False
    ...
    if missing_images:
        print(f"Предупреждение: Отсутствуют обязательные изображения: {missing_images}")
